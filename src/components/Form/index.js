...
const FormComponent = () => {
    ...
    (values) => {
      dispatch(
        createToDoThunk({
          ...
          successCallback: () => navigate("/todos"),
        })
      );
    },
    [dispatch, navigate]
  ...
  const handleFormChange = useCallback(
    (_, allFields) => {
      const formIsValid = allFields.every(
        (field) =>
          field.name[0] !== "description" || (field.value && field.value.trim())
      );
  ...
  return (
    ...
    >
      ...
      <Form.Item label="Description" name="description" rules={[validator()]}>
