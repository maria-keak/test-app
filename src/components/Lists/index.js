...
const ListsComponent = () => {
  ...
  return (
    <Styled.Content style={{ width: "100%" }}>
      {todos.length ? (
        todos.map((todo) => {
          return (
            <Styled.Item key={todo.id + "_i"}>
              <Checkbox
                checked={todo.done}
              ...
              <Styled.Actions>
                <Link to={"/todo/" + todo.id}>See more</Link>
        ...
        <p>
          The To-Do List is currently empty. Please tap 'Create New Todo' to get
          started.
        </p>
      )}
    </Styled.Content>
