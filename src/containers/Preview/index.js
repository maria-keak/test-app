...
const PreviewToDo = () => {
  ...
  useEffect(() => {
    dispatch(
      previewToDoThunk({
        id,
        successCallback: (finded) => {
          setTodo(finded);
          setodoStatus(finded.done);
        },
      })
    );
  }, []);
  ...
  const handleDelete = useCallback(
    (todo) => {
      dispatch(
        deleteItemThunk({ id: todo.id, callback: () => navigate("/todos") })
      );
    },
    [dispatch]
  );
    ...
    <CardComponent
      ...
      content={
        <div className="content">
          {todo ? (
            <>
              <TodoIdText>Todo ID {todo.id}</TodoIdText>
              <div>
                <Desc>{todo.description}</Desc>
                <StatusText isDone={todoStatus}>
                  {todoStatus ? "Done" : "Not Done"}
                </StatusText>
                <StyledButton
                  type={todoStatus ? "dashed" : "primary"}
